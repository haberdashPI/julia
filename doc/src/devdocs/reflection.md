...
Possible values for `debuginfo` are: `:none`, `:source`, `:default`, and `:full`
(only for [`code_llvm`](@ref)). Per default debug information is not printed, but
that can be switched by setting `Base.IRShow.default_debuginfo[] = :source`.
